...
> 结论性文档。前序排查见 `RAIL-NOT-RENDERING-INVESTIGATION.md`（该文档的 H2 假说命中，且实际根因比 H2 猜想更彻底）。
> 生成时间：2026-09-11 ｜ 对象：`dsh-tidychat` @ 分支 `feat/rail-mirror-and-dots` ｜ 宿主：DSH `0.1.2-rc.1`
...
if (snapshot !== null && snapshot !== undefined && Array.isArray(snapshot.entries)) {
  for (const ev of snapshot.entries) {
    ...
    if (src !== undefined && src !== null && src.kind !== 'user') continue  // 过滤 echo
    let text = ''
    ...
    users.push({ seq: ev.seq, time: /* 事件无 wall-clock 时间，见下方备注 */, summary: String(text).trim().slice(0, 120) })
  }
}
```
...
- `data.source.kind === 'user'` 过滤提交 echo（参照宿主 `dsh-client-connection` L2669）
- 事件**没有 wall-clock 时间戳**：`tip.time`（悬停卡的 `#N · X月X日 HH:mm`）需要换来源——从 DOM 行取（见方案 B），或先置空/去掉该段
...
| `measurePos()` L1400-1406 | 右缘模式 gutter 优先取 composer 参照，输入框更贴右时被低估（本次 gutter=94 属运气） | 按 `navSide` 取更合适参照，或对左右两侧取 min |
...
2. 打开有历史消息的会话 → 会话区右缘应出现半透明蓝色圆点列（当前配置 `navSide: right` + `navStyle: dot`）
3. 悬停出现摘要卡、点击跳转、滚动时当前轮高亮
4. 新建空白会话 → 轨隐藏（users=0 属正常设计，此时不可作为失败判据）
5. 顺手确认 `window.__tidychatRailDebug`（若探针未删）`users > 0`、`wouldRender: true`
...
> （规划产物：`.agents/plans/tidychat-rail-dom-source/`，已批准并实施）。
...
| measurePos gutter 修正 | 参照候选 = 输入框卡片 + 首个/末个会话行，按侧别取最贴边（右缘 maxRight / 左缘 minLeft），防右缘轨被误隐藏 | ~L1437-1453 |
...
- `pnpm typecheck` + `pnpm build` ✅ 通过（lib/client.js 73.40 kB）
- 浏览器端 Must Pass 验证：待硬刷新后按 `.agents/plans/tidychat-rail-dom-source/checklist.md` 逐项执行
