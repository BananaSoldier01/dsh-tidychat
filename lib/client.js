...
function apply(ctx) {
	...
	const reportAndOpenIssue = (tags) => {
		...
		const title = `[问题报告] ${tags.length > 0 ? tags.join("、") : issues.length > 0 ? "检测到异常" : "问题反馈"}（插件 v0.2.0）`;
		try {
	...
	const hueColor = (hue, light, fallback) => {
		if (typeof hue === "string") {
			...
			if (palette !== void 0) return palette[NAV_LIGHT_IDX[typeof light === "string" ? light : "l2"] ?? 1];
		}
		return fallback;
	};
	const parseRgb = (s) => {
		...
		if (m !== null) {
			if ((m[4] !== void 0 ? Number(m[4]) : 1) === 0) return null;
			return [
				Number(m[1]),
				Number(m[2]),
				Number(m[3])
	...
	const isDarkBackground = () => {
		try {
			const candidates = [];
			const container = findScrollContainer();
			if (container !== null) candidates.push(container);
	...
	const resolveNavColors = () => {
		const brand = getComputedStyle(document.documentElement).getPropertyValue("--dsw-alias-state-business-primary").trim() || "#3b82f6";
		let bar;
		if ((config.navColor ?? "auto") === "auto") bar = isDarkBackground() ? "rgba(226,226,226,0.85)" : "rgba(80,80,80,0.78)";
		else bar = hueColor(config.navColor, config.navColorLight, "rgba(128,128,128,0.8)");
		const hot = hueColor(config.navAccent, config.navAccentLight, brand);
		return {
			bar,
			hot
		};
		...
		const readConfig = () => {
			try {
				...
				if (snap !== null && snap !== void 0 && snap.status === "ready" && snap.value) {
					...
					config.navColorLight = typeof snap.value.navColorLight === "string" ? snap.value.navColorLight : "l2";
					config.navAccent = typeof snap.value.navAccent === "string" ? snap.value.navAccent : "blue";
					config.navAccentLight = typeof snap.value.navAccentLight === "string" ? snap.value.navAccentLight : "l2";
				}
			} catch {}
		};
		readConfig();
	...
	const TidychatSettingsCard = () => {
		...
		return react.createElement("li", { className: "tidychat-card" + (open ? " tidychat-card-open" : "") }, react.createElement("button", {
			...
			key: "navColor",
			className: "tidychat-field"
		}, react.createElement("div", { className: "tidychat-field-head" }, react.createElement("span", { className: "tidychat-field-label" }, "定位条默认色")), react.createElement("div", { className: "tidychat-color-sub" }, react.createElement("span", { className: "tidychat-color-sub-label" }, "色系"), chipRow([{
			key: "auto",
			label: "自动",
			preview: "linear-gradient(135deg, #222 50%, #f2f2f2 50%)"
		}, ...NAV_HUE_OPTIONS], String(value.navColor ?? "auto"), (k) => setColor("navColor", k), !writable)), react.createElement("div", { className: "tidychat-color-sub" }, react.createElement("span", { className: "tidychat-color-sub-label" }, "明度"), chipRow(NAV_LIGHT_OPTIONS, String(value.navColorLight ?? "l2"), (k) => setColor("navColorLight", k), !writable || (value.navColor ?? "auto") === "auto")), react.createElement("p", { className: "tidychat-field-hint" }, "自动 = 跟随聊天区背景明暗自动选择对比灰；手动 = 按「色系 × 明度」着色。")), react.createElement("div", {
			key: "navAccent",
			className: "tidychat-field"
		}, react.createElement("div", { className: "tidychat-field-head" }, react.createElement("span", { className: "tidychat-field-label" }, "强调色（当前 / 悬停回合）")), react.createElement("div", { className: "tidychat-color-sub" }, react.createElement("span", { className: "tidychat-color-sub-label" }, "色系"), chipRow(NAV_HUE_OPTIONS, String(value.navAccent ?? "blue"), (k) => setColor("navAccent", k), !writable)), react.createElement("div", { className: "tidychat-color-sub" }, react.createElement("span", { className: "tidychat-color-sub-label" }, "明度"), chipRow(NAV_LIGHT_OPTIONS, String(value.navAccentLight ?? "l2"), (k) => setColor("navAccentLight", k), !writable)), react.createElement("p", { className: "tidychat-field-hint" }, "当前轮次与悬停 / 导航目标回合以强调色高亮；普通条幅用默认色。")), react.createElement("div", {
			key: "report",
