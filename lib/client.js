...
function apply(ctx) {
	...
	const snapshotUserTurns = () => {
		...
		try {
			...
			if (snap === null || snap === void 0 || !Array.isArray(snap.entries)) return -1;
			let n = 0;
	...
	const reportAndOpenIssue = (tags) => {
		...
		const title = `[问题报告] ${tags.length > 0 ? tags.join("、") : issues.length > 0 ? "检测到异常" : "问题反馈"}（插件 v0.2.9）`;
		try {
	...
	ctx.slots.inject("conversation.session.header.utilities", () => ctx.slots.register({
		...
		const users = [];
		if (snapshot !== null && snapshot !== void 0 && Array.isArray(snapshot.entries)) for (const entry of snapshot.entries) {
			if (entry === null || entry === void 0 || entry.type !== "event") continue;
			const ev = entry.event;
			if (ev === null || ev === void 0 || ev.type !== "user/message") continue;
			const src = ev.data?.source;
