...
function apply(ctx) {
	...
	const reportAndOpenIssue = (tags) => {
		...
		const title = `[问题报告] ${tags.length > 0 ? tags.join("、") : issues.length > 0 ? "检测到异常" : "问题反馈"}（插件 v0.2.1）`;
		try {
	...
	const resolveTipColors = () => {
		...
		if (bgRgb === null) return {
			text: primary,
			head: secondary
		};
		const darkBg = .2126 * bgRgb[0] + .7152 * bgRgb[1] + .0722 * bgRgb[2] < 128;
		const fix = (candidate) => {
