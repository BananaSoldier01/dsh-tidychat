...
function apply(ctx) {
	...
	const reportAndOpenIssue = (tags) => {
		...
		const title = `[问题报告] ${tags.length > 0 ? tags.join("、") : issues.length > 0 ? "检测到异常" : "问题反馈"}（插件 v0.2.10）`;
		try {
	...
	const TidychatGuide = () => {
		...
		react.useEffect(() => {
			const fn = () => setGuideTick((t) => t + 1);
			guideListeners.push(fn);
			return () => {
				const i = guideListeners.indexOf(fn);
				if (i >= 0) guideListeners.splice(i, 1);
			};
		}, []);
		react.useEffect(() => {
			if (settingsScope === null) {
		...
		const seen = snap !== null && snap !== void 0 && snap.value?.navGuideSeen === true;
		react.useEffect(() => {
			if (seen) return;
			if (officialRailPresent()) {
				setOfficial(true);
				return;
			}
		...
		if (snap === null || snap === void 0) return null;
		...
		const pick = (patch) => {
			...
			settingsScope.set("navGuideSeen", true).catch(() => {});
			for (const [field, v] of Object.entries(patch)) settingsScope.set(field, v).catch(() => {});
		};
		return react.createElement("div", { className: "tidychat-guide-wrap" }, react.createElement("div", {
			className: "tidychat-guide",
			role: "dialog",
			"aria-label": "定位条选择向导"
		}, react.createElement("p", { className: "tidychat-guide-title" }, "现在有两条导航轨，选一条就行"), react.createElement("div", { className: "tidychat-guide-body" }, react.createElement("div", null, "· ", react.createElement("b", null, "左缘"), "：本插件（可贴左/贴右、横线/圆点、外圈、调色盘）"), react.createElement("div", null, "· ", react.createElement("b", null, "右缘"), "：DSH 官方 TurnNavigator（0.1.2+ 自带）"), react.createElement("div", null, "两者取其一或都留着都行；开关都在「设置 → 插件配置 → 会话整理tidychat」里，随时可改。")), react.createElement("div", { className: "tidychat-guide-actions" }, react.createElement("button", {
			type: "button",
			className: "tidychat-guide-btn tidychat-guide-btn-primary",
			onClick: () => pick({
				navigator: true,
				hideOfficialNav: true
			})
		}, "用插件的，隐藏官方轨"), react.createElement("button", {
			type: "button",
		...
		}, "用官方的，关掉插件轨"), react.createElement("button", {
			type: "button",
			className: "tidychat-guide-btn",
			onClick: () => pick({
				navigator: true,
		...
		}, "两条都留着"), react.createElement("button", {
			type: "button",
			className: "tidychat-guide-dismiss",
			onClick: () => pick({})
		}, "知道了"))));
	};
	...
	const TidychatSettingsCard = () => {
		...
		const fieldsTop = [
			...
			[
				"navigator",
				"定位条",
				"聊天区边缘的细窄条状导航，悬停显示摘要、点击跳转到对应消息；贴边与样式可在下方调整。"
			]
		];
		const fieldsBottom = [[
			"hideOfficialNav",
		...
		const toggleField = ([field, label, hint]) => react.createElement("div", {
			key: field,
		...
		return react.createElement("li", { className: "tidychat-card" + (open ? " tidychat-card-open" : "") }, react.createElement("button", {
		...
		}, react.createElement("span", { className: "tidychat-card-headtext" }, react.createElement("span", { className: "tidychat-card-name" }, "会话整理tidychat"), react.createElement("span", { className: "tidychat-card-desc" }, "折叠、分隔线、定位条 —— 把长会话整理成可扫读的结论流")), react.createElement("svg", {
		...
		}))), open ? react.createElement("div", { className: "tidychat-card-body" }, fieldsTop.map(toggleField), react.createElement("div", {
			key: "navLayout",
			className: "tidychat-field"
			...
			className: "tidychat-field-head",
			style: { marginTop: "8px" }
		}, react.createElement("span", { className: "tidychat-field-label" }, "外圈")), chipRow(NAV_RING_OPTIONS, value.navRing === true ? "on" : "off", (k) => setColor("navRing", k === "on"), !writable), react.createElement("p", { className: "tidychat-field-hint" }, "位置 = 消息轨贴会话区左缘或右缘，右缘时整体镜像（横线模式的强调三角指左、摘要卡从左侧弹出）；样式 = 横线或圆点，圆点模式同样保留悬停鱼眼放大与点击跳转；外圈 = 在当前轮与悬停轮的标记外描一圈强调色（1px、外扩 2px），横线为胶囊形、圆点为正圆环，颜色跟随下方「强调色」。"), react.createElement("button", {
			type: "button",
			className: "tidychat-guide-btn",
		...
		}, "重新显示首次引导")), fieldsBottom.map(toggleField), react.createElement("div", {
			key: "navColors",
			className: "tidychat-field"
