...
function apply(ctx) {
	...
	try {
		new PerformanceObserver((list) => {
			for (const entry of list.getEntries()) lastLongTaskMs = Math.max(lastLongTaskMs, Number(entry.duration ?? 0));
		}).observe({
			type: "longtask",
			buffered: true
		});
	...
	function settleThenMeasure(sessionId, gen, before) {
		...
		const finish = (isTimeout) => {
			...
			if (sessionId !== activeSessionId) return;
			const st = governor.get(sessionId);
			if (st === void 0 || st.generation !== gen || st.status !== "loading") return;
			...
			if (lastLongTaskMs > st.longTaskBaseline) {
				pauseGovernor(st);
				return;
			}
