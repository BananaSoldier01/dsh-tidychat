...
function apply(ctx) {
	...
	ctx.effect(() => {
		try {
			const po = new PerformanceObserver((list) => {
				longTaskSeq += list.getEntries().length;
			});
			po.observe({
				type: "longtask",
				buffered: true
	...
	const findLoadOlderButton = () => {
		const scope = findScrollContainer() ?? document;
		const buttons = Array.from(scope.querySelectorAll("button"));
		for (const b of buttons) {
			const t = (b.textContent || "").trim();
			if (t === "加载更早" || t === "加载更多" || t.indexOf("Load older") === 0 || t.indexOf("Load more") === 0) return b;
		}
		return null;
	};
	const countAnchors = () => {
		return (findScrollContainer() ?? document).querySelectorAll("[data-chat-anchor-key]").length;
	};
	const measuredScan = () => {
	...
	function scheduleNext(sessionId) {
		...
		const w = window;
		if (typeof w.requestIdleCallback === "function") w.requestIdleCallback(run, { timeout: 2e3 });
	...
	function settleThenMeasure(sessionId, gen, before) {
		...
		const finish = (isTimeout) => {
			...
			const st = governor.get(sessionId);
			if (st === void 0 || st.generation !== gen || st.status !== "loading") {
				governorBusy = false;
				return;
			}
			...
			if (longTaskSeq > st.longTaskSeqBase) {
				pauseGovernor(st);
				return;
			}
