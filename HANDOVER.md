...
> 面向：接续开发的新会话 / 新协作者。内容基于 **v0.2.8（main）** 快照。仓库根目录：`/Users/wuke/工作文件/DeepSeek_Harness/dsh-tidychat`（本机 link 模式开发）。
...
- npm：`@bananasoldier01/dsh-tidychat`（public，最新 **0.2.8**）
- 插件索引：**awesome-dsh-plugin 已收录**（PR #3067 合并，session 分类 + 截图），即 dsh-market 源
- 当前版本线：v0.2.0 → v0.2.8（0.2.0 导航条大版本；0.2.1 配色；0.2.2 提示卡可读性；0.2.3 配色/publish 准备；0.2.4 npm 元数据；0.2.5 Hardening；0.2.6 折叠/分隔线重做 + 左缘定位条暂缓；0.2.7 settings API 向后兼容；0.2.8 旧版 DSH 折叠回退 + 左缘定位条确认可用）
